...
def intraday():
	...
	while total_calls_a_day<75:	
		...
		for c in intraday_list:
			...
			data.to_csv('newcompanylist/{fname}/intraday trading/{fname}_intraday.csv'.format(fname = c))
			company = '{fname}'.format(fname = c)

			technical_indicators_intraday = pd.read_csv('newcompanylist/{fname}/intraday trading/{fname}_intraday.csv'.format(fname = c))
			...
			xForRange = technical_indicators_intraday['Close'].tail(5).tolist()
			...
			pd.DataFrame(swingupper).to_csv('newcompanylist/{fname}/intraday trading/{fname}_upperband.csv'.format(fname = c))
			pd.DataFrame(swingmiddle).to_csv('newcompanylist/{fname}/intraday trading/{fname}_middleband.csv'.format(fname = c))
			pd.DataFrame(swinglower).to_csv('newcompanylist/{fname}/intraday trading/{fname}_lowerband.csv'.format(fname = c))


			upperband = pd.read_csv('newcompanylist/{fname}/intraday trading/{fname}_upperband.csv'.format(fname = c))
			upperbandvalue = upperband['0'].tail(5)
			upperbandvalue = upperbandvalue.mean()

			currupperbandvalue = upperband['0'].iloc[-1]

			middleband = pd.read_csv('newcompanylist/{fname}/intraday trading/{fname}_middleband.csv'.format(fname = c))
			...
			lowerband = pd.read_csv('newcompanylist/{fname}/intraday trading/{fname}_lowerband.csv'.format(fname = c))
			...
			pd.DataFrame(rsi_value).to_csv('newcompanylist/{fname}/intraday trading/{fname}_rsi_value.csv'.format(fname = c))

			retrieveRSI = pd.read_csv('newcompanylist/{fname}/intraday trading/{fname}_rsi_value.csv'.format(fname = c))
			...
			pd.DataFrame(adx_value).to_csv("newcompanylist/{fname}/intraday trading/{fname}_adxvalue.csv".format(fname=c))

			retrieveadx = pd.read_csv("newcompanylist/{fname}/intraday trading/{fname}_adxvalue.csv".format(fname=c))
			...
			pd.DataFrame(ema_value_5).to_csv('newcompanylist/{fname}/intraday trading/{fname}_ema_value_5.csv'.format(fname = c))
			pd.DataFrame(ema_value_8).to_csv('newcompanylist/{fname}/intraday trading/{fname}_ema_value_8.csv'.format(fname = c))
			pd.DataFrame(ema_value_13).to_csv('newcompanylist/{fname}/intraday trading/{fname}_ema_value_13.csv'.format(fname = c))
			

			retrieveEma_5 = pd.read_csv('newcompanylist/{fname}/intraday trading/{fname}_ema_value_5.csv'.format(fname = c))
			currretrieve_ema_value_5 = retrieveEma_5['0'].iloc[-1]
			pastfiveema5 = retrieveEma_5['0'].head(-1).tail(5).mean()

			retrieveEma_8 = pd.read_csv('newcompanylist/{fname}/swing trading/{fname}_ema_value_8.csv'.format(fname = c))
			currretrieve_ema_value_8 = retrieveEma_8['0'].iloc[-1]
			pastfiveema8 = retrieveEma_8['0'].head(-1).tail(5).mean()


			retrieveEma_13 = pd.read_csv('newcompanylist/{fname}/swing trading/{fname}_ema_value_13.csv'.format(fname = c))
			currretrieve_ema_value_13 = retrieveEma_13['0'].iloc[-1]
			pastfiveema13 = retrieveEma_13['0'].head(-1).tail(5).mean()
			...
			if(flag): #this line of code checks whether there is the same closing price in the list which i want to remove. 
				if(flagVolume): #this line of code checks whether there is the same volume in the list which i want to remove. The assumption is it won't be unless the value of volume is 0.
					...
					if retrieve_rsi_value<=35:
						if lastprice<currlowerbandvalue:
							lowPointliststrategy = (company,lastprice)
							lowPointlist.append(lowPointliststrategy)
							lowPointlist.sort(key = sortSecond, reverse = False)


					if  currretrieve_ema_value_5> currretrieve_ema_value_8 and currretrieve_ema_value_5> currretrieve_ema_value_13 and currretrieve_ema_value_8> currretrieve_ema_value_13:	
						if pastfiveema5< pastfiveema8 and pastfiveema5< pastfiveema13 and pastfiveema8< pastfiveema13:
		...
		print('-------------------------------------------------------BOLLINGER-CROSSOVER STRATEGY-------------------------------------------------------')
		...
		print('-------------------------------------------------------LOWEST POINT STRATEGY-------------------------------------------------------')		
		lowPointlistObj = pd.DataFrame(lowPointlist, columns = ['Company','Close']) 
		#lowPointlistObj .to_sql(con=database_connection, name='intradaybbsqueeze', if_exists='replace') 
		print(lowPointlistObj)
